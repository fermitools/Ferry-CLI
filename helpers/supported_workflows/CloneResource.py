    ...
    def run(self: "CloneResource", api:'FerryAPI', args: Any)->Any:  # type: ignore
        ...
        try:
            resources =  {
                            ...
                            for resource 
                            in self.verify_output(api.call_endpoint("getAllComputeResources")) 
                            if resource.get("resourcename", "") in [args["clone"], args["new_resource"]]
                        }
            print(resources)
            ...
            if args["new_resource"] not in resources:
                print(f"New resource doesn't exist. Creating new resource with the same attributes as: {args['clone']}")
                self.verify_output(api.call_endpoint("createComputeResource", method='PUT', params=cloned_resource_data))
            else:
                # If the clone already exists, we will update its info
                self.verify_output(api.call_endpoint("setComputeResourceInfo", method='POST', params=cloned_resource_data))


            # Now we will get all user groups, and filter for the original resource
            group_json = self.verify_output(api.call_endpoint( 
            ...
            if resources:
                ...
                for resource in resources:
                    ...
                    for user in resource.get("users", []):
                        ...
                        print(f"Updating user access to {args['new_resource']} for {user['username']}")
                        if "status" in user_access_data:
                            del user_access_data["status"]
                        self.verify_output(api.call_endpoint("setUserAccessToComputeResource", method="PUT", params=user_access_data))
            
            print(f"Resource '{args['clone']}' has been successful cloned as '{args['new_resource']}'")
            exit(0)
