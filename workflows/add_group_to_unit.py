...
parser = ArgumentParser(description="Custom Ferry function to create, and add a group to a unit, and add resources to it")
...
parser.add_argument('-p', '--primary', help="True if this is the primary group for the affiliation - default(false)", default=False)
parser.add_argument('-r', '--resources', nargs='*', help="Resources to be added to (getPasswdFile)", default=None)
...
if args.resources is not None:
    ...
    for resource in args.resources:
        resp = eval(ferry_api.execute_endpoint("getPasswdFile", [f"--groupname={args.groupname}", f"--unitname={args.unitname}",f"--resourcename={resource}"]))
        output = resp.get("ferry_output", None) if resp else None
        if output and args.unitname in output:
            if "resources" in output[args.unitname] and resource in output[args.unitname]["resources"]:
                for user in output[args.unitname]["resources"][resource]:
                    resp = json.loads(ferry_api.execute_endpoint("setUserAccessToComputeResource", [f"--groupname={args.groupname}",
                    ...
                    if resp and "ferry_status" in resp and resp["ferry_status"] == "success":
                        print(f"Added User {user.get('username', '')} to resource: {resource}")
