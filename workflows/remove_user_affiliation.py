...
resp = eval(
    ferry_api.execute_endpoint(
        "getUserAccessToComputeResources", [f"--username={args.username}"]
    )
)
...
user_fqans = eval(
    ferry_api.execute_endpoint(
        "getUserFQANs", [f"--unitname={args.unitname}", f"--username={args.username}"]
    )
).get("ferry_output")
...
user_groups = eval(
    ferry_api.execute_endpoint("getUserGroups", [f"--username={username}"])
).get("ferry_output")
...
ferry_api.execute_endpoint(
    "/setUserGridAccess", [f"--unitname={unitname}", f"--username={username}"]
)

cert_dns = eval(
    ferry_api.execute_endpoint(
        "getUserCertificateDNs", [f"--unitname={unitname}", f"--username={username}"]
    )
).get("ferry_output")
